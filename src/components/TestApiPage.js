...
import '../App.css';
import { getPostsUsingAwait, getPostsUsingThen, deleteSomethingUsingThen, getAllPosts } from '../Services/apiCalls';
...
function TestApiPage() {
		...
		try {
			getPostsUsingThen(postID).then((response) => {
				...
				if (response.constructor === objectConstructor) {
					//remove this ones integrated with webservice
	...
	return (
        ...
        ></input> */}
				<TextField id="standard-input-id" label="ID" type="text" onChange={(e) => setpostID(e.target.value)} />
			</div>
        ...
        <div>{viewResult.body}</div>
			...
			{viewResult.map((element, index) => <PostList item={element} key={index} />)}
		</React.Fragment>
	);
}
